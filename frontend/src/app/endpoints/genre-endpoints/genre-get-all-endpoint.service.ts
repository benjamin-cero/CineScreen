...
export interface GenreGetAllRequest extends MyPagedRequest {
  q?: string;
}

export interface GenreGetAllResponse {
  id: number;
  name: string;
}
...
export class GenreGetAllEndpointService implements MyBaseEndpointAsync<GenreGetAllRequest, MyPagedList<GenreGetAllResponse>> {
  private apiUrl = `${MyConfig.api_address}/genres/filter`;

  constructor(private httpClient: HttpClient, private cacheService: MyCacheService) {
  }

  handleAsync(request: GenreGetAllRequest, useCache: boolean = false, cacheTTL: number = 300000) {
    ...
    if (useCache && this.cacheService.has(cacheKey)) {
      let data = this.cacheService.get<MyPagedList<GenreGetAllResponse>>(cacheKey)!;
      console.log(cacheKey + " use cached: " + data.dataItems.length)
      return of(data);
    }

    const params = buildHttpParams(request);  // Use the helper function here
    return this.httpClient.get<MyPagedList<GenreGetAllResponse>>(`${this.apiUrl}`, {params}).pipe(
      tap((data) => {
        if (useCache) {
